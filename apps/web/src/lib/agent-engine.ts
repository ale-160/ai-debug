...
下方"## 当前画布"段落由系统在每次调用前注入，包含项目名、节点数、最近节点预览、选中节点路径。请基于该状态给出针对性建议：
- 画布为空 → 引导用户从零开始
- 已有节点 → 引用具体节点内容，避免重复提问
 ...
 * - 项目为空（草稿态）→ 提示引导用户从零开始
 * - 有项目无节点 → 提示首条消息将派生项目名
 * - 有节点 → 列出最近 5 个节点预览 + 选中节点路径
 *
 * 返回空字符串表示无需注入（理论上不会出现，因为始终有项目状态可描述）。
 */
export function buildCanvasContextText(snapshot: CanvasContextSnapshot): string {
  ...
  lines.push(`- 项目：${snapshot.projectName ?? '未命名'}（${snapshot.nodeCount} 个节点）`);

  if (snapshot.selectedNodeId) {
    lines.push(`- 选中节点：${snapshot.selectedNodeId}`);
    if (snapshot.selectedPathPreview.length > 0) {
      lines.push('- 选中节点路径（从根到当前）：');
      snapshot.selectedPathPreview.forEach((n, i) => {
        lines.push(`  ${i + 1}. [${n.id.slice(0, 8)}] ${n.userMessagePreview}`);
      });
    }
  } else {
    lines.push('- 选中节点：无（新节点将作为根或从最近节点分叉）');
  }
  ...
  if (snapshot.recentNodes.length > 0) {
    ...
    snapshot.recentNodes.forEach((n, i) => {
      ...
      lines.push(`  ${i + 1}. [${n.id.slice(0, 8)}] ${branch}${statusTag} ${n.userMessagePreview}`);
    });
  }
...
export async function streamAssistantResponse(
  ...
  const { skillId, skills, history, canvasContext, autoCreateNode, signal, onDelta, onForwarded } =
    options;
  ...
  try {
    ...
    if (forwardedTexts.length > 0 && onForwarded) {
      ...
      // 这样用户每次和助手对话都会在画布留下痕迹，形成"对话即建图"的工作流。
      forwardedTexts.push(userText.trim());
      void (async () => {
        onForwarded(userText.trim());
        await Promise.resolve();
      })();
    }
