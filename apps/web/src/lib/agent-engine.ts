...
- 用户描述了一个明确的问题想交给 AI 分析
- 你帮用户整理出了一段结构化的提问
- 用户说"把这个放到画布"/"创建节点"/"新建分支"等指令
...
export async function streamAssistantResponse(
  ...
  try {
    ...
    // 扫描转发标记（异步触发，不阻塞返回）
    // P0-3：对转发文本做兜底清洗，剥离 LLM 误加的角色前缀/代码块包裹
    const forwardedText = parseForwardedText(fullText);
    if (forwardedText && onForwarded) {
      const sanitized = sanitizeForwardedText(forwardedText);
      // 异步触发，避免阻塞引擎返回
      void Promise.resolve().then(() => onForwarded(sanitized));
    }

    return {
      status: 'success',
      text: fullText,
      forwardedText: forwardedText ? sanitizeForwardedText(forwardedText) : forwardedText,
    };
  } catch (err) {
    // 用户主动取消（AbortError）
