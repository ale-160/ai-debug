...
export interface EvolutionBranchResult {
  ...
  stopped: boolean;
  /** 该路遇到的错误信息（非 AbortError，已转为 stopped 不向上抛出） */
  error?: string;
}
 ...
 *
 * 3.10.2：在 validChildren 过滤时直接检查 status，避免"先递归再排除"的浪费。
 * 入口处的 abandoned 检查仅用于根节点（防止根本身就是 abandoned 时返回 []）；
 * 子节点通过 validChildren 过滤后再递归，不会进入 abandoned 子树。
 *
 * @param nodeId  选中节点 id（推演起点子树的根）
 * @param nodes   全部节点
 * @returns       有效叶节点 id 列表（多路并行推演的起点）
 */
export function findEffectiveLeaves(nodeId: string, nodes: Node<TurnNodeData>[]): string[] {
  ...
  const dfs = (id: string) => {
    ...
    // 3.10.2：该检查主要用于根节点（root 自身可能 abandoned）。
    // 子节点已通过 validChildren 过滤，理论上不会进来；保留作为兜底防御。
    if (node.data.status === 'abandoned') return;

    const children = childrenMap.get(id) ?? [];
    // 3.10.2：在入队前直接过滤 abandoned 子节点，避免"先递归再排除"的浪费
    const validChildren: string[] = [];
    for (const cid of children) {
      const child = nodeMap.get(cid);
      if (child && child.data.status !== 'abandoned') {
 ...
 *
 * 3.7.3：CONVERGED 检测收紧为"独立占一行"，避免误匹配 LLM 回答中提到该词的情况。
 */
function parseEvolutionCandidates(
  ...
  // 3.7.3：CONVERGED 必须独占一行（大小写不敏感），避免误匹配 LLM 回答中提到该词。
  // 兼容首尾空白与代码块包裹场景：去掉 ``` 包裹后再按行匹配。
  const unstripped = trimmed.replace(/^```|```$/g, '').trim();
  const convergedLine = unstripped.split(/\r?\n/).some((line) => line.trim() === 'CONVERGED');
  if (convergedLine) {
...
function tryParseCandidates(raw: string, maxBranches: number): EvolutionCandidate[] {
  try {
 ...
 */
function buildEvolutionPrompt(currentNode: Node<TurnNodeData>, maxBranches: number): string {
...
export async function runEvolutionStep(
  ...
  const { maxBranches, startNodeId, step, extraContext, onDelta, userDirection } = options;

  // 3.7.2：捕获调用时的 projectId，await 后校验避免切换项目后写入错项目
  ...
  // 3.7.2：await 后校验项目未变化，避免切换项目后继续在错项目上创建节点
  if (useDebugStore.getState().currentProjectId !== projectIdAtCall) {
  ...
  const { createTurnNode, updateTurnNode, appendAssistantChunk, flushStreamBuffer } = store;
  ...
  // 3.7.2：获取最新 nodes 前再次校验项目未变化（await 期间用户可能切项目）
  if (useDebugStore.getState().currentProjectId !== projectIdAtCall) {
    const err = new Error('Aborted');
    err.name = 'AbortError';
    throw err;
...
export async function runEvolution(
  ...
  // 3.10.1：共享暂停门 + 互斥锁，保证 onLowConfidence 串行执行且其他路暂停下一步。
  // 原实现：onLowConfidence 仅阻塞当前路，其他路继续推进，可能导致多个弹窗并发、
  // 用户决策与正在进行的 LLM 调用交错。改为：任一路触发 onLowConfidence 时，
  // 其他路在进入下一步前等待；onLowConfidence 调用本身通过 mutex 串行化。
  let pausePromise: Promise<void> | null = null;
  let pauseResolve: (() => void) | null = null;
  // 串行化 onLowConfidence 调用的链式 Promise
  let lowConfidenceChain: Promise<void> = Promise.resolve();

  /** 暂停所有路：在触发 onLowConfidence 前调用，其他路进入下一步前会等待 */
  const pauseAll = () => {
    if (pausePromise) return; // 已暂停
    pausePromise = new Promise<void>((r) => {
      pauseResolve = r;
    });
  };
  /** 恢复所有路：onLowConfidence 决策返回后调用 */
  const resumeAll = () => {
    if (pauseResolve) {
      pauseResolve();
      pausePromise = null;
  ...
  const waitIfPaused = async () => {
    // while 循环防止 resumeAll 后又被另一路 pauseAll 的极端时序
    while (pausePromise) {
      await pausePromise;
    }
  };
  /**
   * 串行化执行 onLowConfidence：暂停其他路 → 调用回调 → 恢复其他路。
   * 多路同时触发时通过 lowConfidenceChain 排队，避免并发弹窗。
   */
  const runLowConfidenceSerial = async (
    nodeId: string,
    conf: number,
    reasoning: string,
  ): Promise<'continue' | 'stop' | { newDirection: string }> => {
    // 排队等待前一个 onLowConfidence 完成
    const prev = lowConfidenceChain;
    let release!: () => void;
    lowConfidenceChain = new Promise<void>((r) => {
      release = r;
    });
    await prev;
    // 进入临界区：暂停其他路 → 调用回调 → 恢复其他路
    pauseAll();
    try {
      return await onLowConfidence!(nodeId, conf, reasoning);
    } finally {
      resumeAll();
      release();
    }
   ...
   */
  const branchTask = async (startNodeId: string): Promise<EvolutionBranchResult> => {
    ...
    try {
      while (step < maxSteps && !signal.aborted) {
        // 3.10.1：等待其他路的 onLowConfidence 决策完成，避免并发推进
        await waitIfPaused();
        ...
        if (bestConf < confidenceThreshold && onLowConfidence) {
          ...
          if (typeof decision === 'object' && decision !== null && 'newDirection' in decision) {
      ...
      if (err instanceof Error && err.name === 'AbortError') {
        ...
        // 其他错误：转为 stopped 结果返回，不向上抛出，避免 Promise.allSettled reject 丢失其他路结果
  ...
  const settled = await Promise.allSettled(startNodeIds.map((id) => branchTask(id)));
